# Hi 👋, I'm Devi Pavithra R

### Full Stack Developer | AI Enthusiast | Problem Solver

📍 Tenkasi, Tamil Nadu, India  
📧 devipavithra12603@gmail.com  
📱 +91 9487801101

---

## 🚀 About Me

Passionate Full Stack Developer with experience in designing, developing, and deploying scalable web applications. Skilled in React, Laravel, PHP, Python, .NET, Node.js, Android Development, and AI-powered solutions.

- 💻 Full Stack Web Development
- 📱 Android App Development
- 🤖 AI & Machine Learning Applications
- 🏗️ Microservices & Micro Frontend Architecture
- 🐳 Docker & CI/CD Pipelines
- 📊 Database Design & Optimization
- 🌱 Currently Learning Kubernetes & Cloud Technologies

---

## 🛠️ Tech Stack

## 💻 Tech Stack

<p align="center">
<img src="https://skillicons.dev/icons?i=html,css,js,react,angular,python,php,laravel,nodejs,dotnet,kotlin,flutter,mysql,postgresql,git,docker,postman,vscode,github&perline=6" />
</p>

## 🚀 Tools I Use

<p align="center">
<img src="https://skillicons.dev/icons?i=react,nodejs,python,laravel,docker,mysql,postgresql,kotlin,flutter,git" />
</p>

### Other Skills
- REST APIs
- Retrofit
- MVC Architecture
- ORM
- OOP
- Responsive Design
- CI/CD Pipelines
- Service-Based Architecture

---

## 💼 Professional Experience

### Full Stack Developer
**Arffy Technologies**  
📅 June 2025 – Present

- Developing scalable applications using React, Laravel, Python, .NET, and Node.js.
- Working with Microservices and Micro Frontend Architecture.
- Building Android applications using Kotlin and Flutter.
- Designing and optimizing MySQL and PostgreSQL databases.
- Implementing CI/CD pipelines and Docker-based deployments.
- Collaborating with UI/UX, QA, and DevOps teams.

### PHP Full Stack Developer Intern
**KiteCareer**  
📅 Nov 2024 – May 2025

- Developed dynamic user interfaces using HTML, CSS, and JavaScript.
- Integrated RESTful APIs.
- Optimized MySQL database performance.
- Participated in architecture discussions and code reviews.

---

## 🚀 Featured Projects

### 🏥 Metropolis Hospital Ticketing Tool
**Tech Stack:** PHP, MySQL

- Supported hospital-based ticketing system.
- Managed patient-related utilities.
- Handled support tickets and SLA tracking.

### 📚 DOCX to XML Conversion Tool
**Tech Stack:** Python, Streamlit, Google Vision API

- Converted structured DOCX files into JATS XML.
- Integrated equation extraction using Google Vision API.
- Automated content cleaning and preprocessing.

### 🤖 AI Chatbot & Salon Appointment Agent
**Tech Stack:** React, Python, WebSockets, Ollama

- Built real-time chatbot using LLMs.
- Automated appointment booking workflows.
- Added double-booking prevention logic.

### ☎️ AI Caller Platform
**Tech Stack:** React, Svelte, Python

- Multi-role web application.
- OTP-based authentication.
- Admin, Super Admin, and User workflows.

### 💼 Workaroo Job Portal
**Tech Stack:** React, Node.js, PostgreSQL

- Job seeker and employer platform.
- Dashboard and job posting modules.
- Role-based access control implementation.

### 🛒 Wellness E-Commerce Platform
**Tech Stack:** Laravel, PHP, MySQL

- Product catalog and shopping cart.
- Razorpay payment integration.
- Automated order notifications.

### 🎯 AI Attendance Tracking System
**Tech Stack:** React, Python, YOLOv12, MySQL

- Real-time employee attendance tracking.
- Defaulter analysis dashboard.
- Attendance analytics and reporting.

### 🚚 Truck Logistics Automation
**Tech Stack:** React, Node.js, MySQL

- Concrete mixing and delivery management.
- Dataset migration from MS Access to MySQL.
- Advanced filtering and reporting features.

### 🌍 Jiffy Cultural Exchange Platform
**Tech Stack:** .NET (C#), AngularJS

- Teacher, Staff, and Partner workflows.
- API integration and workflow management.

### 📖 Interactive Company Profile Flipbook
**Tech Stack:** React, CSS3

- Digital company profile.
- Smooth page-flip animations.
- Interactive user experience.

---

## 🎓 Education

### Master of Computer Science
Kamarajar Government Arts College, Surandai

- 2023 – 2025
- Percentage: 87%

### Bachelor of Computer Science
USP Arts and Science College for Women, Kodikurichi

- 2020 – 2023
- Percentage: 86%

---

## 🌟 Strengths

- Strong analytical and problem-solving skills
- Quick learner and adaptable to new technologies
- Language-agnostic development mindset
- Team collaboration and communication
- Passion for scalable and maintainable software solutions

---

## 📈 GitHub Stats


![GitHub Stats](https://github-readme-stats.vercel.app/api?username=DevipavithraR&show_icons=true&theme=github_dark)

![GitHub Streak](https://streak-stats.demolab.com?user=DevipavithraR&theme=github-dark)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=DevipavithraR&layout=compact&theme=github_dark)

---

## 🏆 Coding Profiles

### LeetCode

<a href="https://leetcode.com/u/DeviPavithraR/" target="_blank">
    <img src="https://leetcard.jacoblin.cool/DeviPavithraR?theme=dark&font=Karma&ext=contest" alt="LeetCode Stats"/>
</a>

- 🔥 Active Problem Solver
- 💡 Data Structures & Algorithms Enthusiast
- 🚀 Continuous Learning and Competitive Programming

---

## 🏆 GitHub Trophies

![Trophies](https://github-profile-trophy.vercel.app/?username=YOUR_GITHUB_USERNAME&theme=darkhub)

---

## 🌐 Connect With Me

<p align="left">
<a href="mailto:devipavithra12603@gmail.com">
    <img src="https://img.shields.io/badge/Email-devipavithra12603%40gmail.com-red?style=for-the-badge&logo=gmail">
</a>

<a href="https://www.linkedin.com/in/devi-pavithra-r/">
    <img src="https://img.shields.io/badge/LinkedIn-Devi%20Pavithra%20R-blue?style=for-the-badge&logo=linkedin">
</a>

<a href="https://github.com/DevipavithraR">
    <img src="https://img.shields.io/badge/GitHub-DevipavithraR-black?style=for-the-badge&logo=github">
</a>

<a href="https://leetcode.com/u/DeviPavithraR/">
    <img src="https://img.shields.io/badge/LeetCode-DeviPavithraR-orange?style=for-the-badge&logo=leetcode">
</a>
</p>

---

### 💡 Quote

> "Building scalable applications and continuously learning emerging technologies to create impactful solutions."